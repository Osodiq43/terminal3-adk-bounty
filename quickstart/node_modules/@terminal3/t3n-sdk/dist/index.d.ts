 ...
 * (e.g. each `BoundGrant` inside `OrgContractGrants.grants`) are not
 ...
 * surfaces: the org edge (`org-policy-set-grants` input / `org-grants-get`
 * output) and the flat agent edge (`agent-auth-get` output / `agent-auth-update`
 * input). Stored inline on the unified `ROLE_ASSIGNMENTS_MAP` edge record.
 *
 ...
 * populates only `functions` + `scopes`; the agent edge additionally carries the
 * qualifiers `version_req` / `read_scopes` / `allowed_hosts` and the validity
 * `window`. All qualifier fields are optional and omitted from the wire when
...
/**
 * Agent-auth grants — the standing on-chain authority a user delegates to
 * an agent (envelope-free authorisation).
 *
 ...
 * `grantee` is the agent DID it authorises, so there is no per-agent grouping —
 * a user's whole delegation surface is the single `grants` array. Reads
...
/**
 * The full agent-auth policy document — the shape of BOTH the `agent-auth-get`
 * output and the `agent-auth-update` input (they are identical). A flat list of
 * grant rows plus the document-level discovery grants; mirrors the contract
 * `AgentAuthDoc` wire shape verbatim (snake_case).
 *
 * `discover_dids` is the list of DIDs the user lets their registered agents
 * *enumerate the contracts of* via `listContracts`: an org DID grants org-mode
 * discovery, the user's own DID grants self-mode discovery. It lives at document
 * level — not on a grant — because discovery must surface contracts the agent
 * cannot yet call. The node's discovery gate additionally requires the calling
 * agent to be the `grantee` of at least one grant (subject binding), so an
 * unregistered agent discovers nothing regardless of this list.
 *
 * The document IS the new state on a write: an empty `grants` list revokes all
 * agent access, and an empty `discover_dids` persists an empty discovery-grant
 * list (there is no per-field merge). Use {@link mergeGrants} /
 * {@link mergeDiscoverDids} to read-merge-write a single grant without dropping
 * the rest.
 */
interface AgentAuthDoc {
    /** Every standing agent-edge grant, each keyed by its `grantee` agent DID. */
    grants: BoundGrant[];
    /** Document-level discovery grants (`did:t3n:<40-hex>` entries). */
    discover_dids: string[];
}
/**
 * Decode an `agent-auth-get` response into an {@link AgentAuthDoc}. The wire is
 ...
 */
declare function parseAgentAuthDoc(response: unknown): AgentAuthDoc;
interface GrantsMergeResult {
    /** Full merged grant list, ready for `agent-auth-update`. */
    grants: BoundGrant[];
...
/** Which standing grant a satisfied/missing entry refers to. */
type DelegationGrantKind = "agent_auth" | "org_grant";
...
/**
 * Response for `delegation.check`: the `agent_auth ∩ org_grant` verdict.
 *
...
declare class T3nClient {
     ...
     *
     * Returns the `agent_auth ∩ org_grant` verdict for `(agent, contract,
     ...
     * The page carries **every member's** activity in that org, not just the
     * caller's own.
     *
     ...
     * 2. `org-remove-member-grants` — sweep the member from the org's
     *    per-contract `BoundGrant` lists, so the capability does not silently
     *    reactivate if the member later rejoins.
     *
     * The member's `agent_auth` delegations are self-scoped (the agent-auth edge in `ROLE_ASSIGNMENTS_MAP` is a
     * per-user document an admin cannot reach), so this cannot sweep them; the host
     * membership gate denies them regardless, and the member's own session can run
     * {@link revokeAgentAuthForOrg} for full cleanup.
     *
     ...
     * 2. `org-remove-member-grants` — sweep the caller's own grants for the org
     *    (the self-revocation branch, `caller == member`).
     * 3. `agent-auth-revoke-org` — drop the caller's own org-scoped agent
     *    delegations.
     *
    ...
    /**
     * Revoke the caller's own agent delegations scoped to one org
     * (detach-cascade `agent_auth` half). Dispatches
     * `agent-auth-revoke-org` on `tee:user/contracts` (**SelfOnly**): every
     * agent-edge grant targeting a `z:<orgTid>:*` contract is removed from the
     * caller's own agent-auth document.
     *
    ...
    /**
     * Read the authenticated user's own agent-auth policy — the standing
     * grants delegated to agents. Dispatches `agent-auth-get` on
     * `tee:authorisations/contracts` (the policy's owning contract), which is
     * **SelfOnly**, so this always returns the caller's own
     * agent-auth edge in `ROLE_ASSIGNMENTS_MAP`.
     *
     * @returns the full policy document as the flat snake_case wire shape: the
     *   `grants` list (each row's `grantee` is the authorised agent DID) plus the
     *   document-level `discover_dids` discovery grants
     * @throws if the session is not authenticated, or the contract refuses
     */
    getAgentAuth(): Promise<AgentAuthDoc>;
    /**
     * Authorise an agent for one contract on the caller's own behalf
     * (envelope-free authorisation). Read-merge-writes the
     * caller's agent-auth edge in `ROLE_ASSIGNMENTS_MAP` via `agent-auth-update`
     * (**SelfOnly**) with `(grantee, contract_id)` union semantics, so other
     * contracts' grants for this or any agent survive untouched.
     *
     * The grant is the flat wire {@link BoundGrant}: its `grantee` is the agent
     ...
     *   `grantee` is the agent DID being authorised (contract_id, functions,
     ...
     * @throws if the session is not authenticated, or the contract refuses
     */
    updateAgentAuth(grant: BoundGrant, options?: {
        discoverDids?: string[];
    }): Promise<{
        preservedRows: string[];
    }>;
     ...
     * target system contract — the agent-auth wrappers dispatch on
     * `tee:authorisations/contracts` (the authorisation policy's owning
    ...
    /**
     * Replace this user's agent-authorisation policy (`tee:authorisations/contracts`
     * → `agent-auth-update`). The supplied document IS the new state: an empty
     * `grants` array revokes all agent access, and an empty `discover_dids`
     * persists an empty discovery-grant list. This is the full-document
     * write — use {@link T3nClient.updateAgentAuth} for the single-grant
     * read-merge-write that preserves other contracts' grants and the
     * standing discovery grants.
     *
     * The {@link AgentAuthDoc} IS the snake_case wire shape — grant rows are
     ...
     */
    agentAuthUpdate(input: AgentAuthDoc): Promise<void>;
    /**
 ...
 *
 * ### Error hygiene (CodeRabbit T3/T4/T5)
 ...
 * underlying network error's message (CodeRabbit T4/T5): a server (or a
...
/**
 * A grant to set via {@link OrgDataClient.setGrants}. The per-grant
 ...
 * `agent_auth` ∩ `org_grant` intersection the host re-reads every call, and
 ...
 * in the Trinity harness; used as the `functions` set for agent-auth grants
...
declare function manifestToTrustAnchor(manifest: SignedTrustManifest): TrustAnchor;
...
export { AGENT_CARDS_SCOPE, AuthMethod, AuthenticationError, BASE_UNITS_PER_TOKEN, ContractResponseError, DEFAULT_ENVIRONMENT, DEFAULT_INDIVIDUAL_THRESHOLD_CENTS, DEFAULT_KYC_POLL_CADENCE, HandshakeError, HttpTransport, InvokeError, KycStatusTimeoutError, LogLevel, MockTransport, NODE_URLS, OrgDataClient, OtpRateLimitedError, RpcError, SessionExpiredError, SessionOrgDataClient, SessionStateError, SessionStatus, SessionUsageReportClient, T3nClient, T3nError, TERMINAL_KYC_STATUSES, TOKEN_DECIMALS, TenantClient, TenantContractsNamespace, TenantMapsNamespace, TenantNamespace, TenantSdkValidationError, TenantTokenNamespace, UnsupportedTenantSdkOperationError, UserUpsertError, WasmError, WireEncodingError, Z_PAYROLL_AUDIT_READ_FUNCTIONS, Z_PAYROLL_RUN_FUNCTIONS, _b64uEncode, assertShape, b64uDecodeStrict, b64uEncodeBytes, buildSecretWriteArgs, bytesToString, canonicalTenantName, clearKeyCache, clearManifestVersionFloor, compactDidFromBytes, createDefaultHandlers, createEmailOtpAuthInput, createEthAuthInput, createLogger, createMlKemPublicKeyHandler, createOidcAuthInput, createOrgDataClientFromSession, createRandomHandler, createUsageReportClientFromSession, createWorkloadAuthInput, decodeWasmErrorMessage, deriveAgentCardEntryId, eth_get_address, extractWasmError, fetchDkgAttestation, fetchMlKemPublicKey, fetchTrustedManifest, formatTokens, generateRandomString, generateUUID, getContractVersion, getEnvironment, getEnvironmentName, getGlobalLogLevel, getLogger, getNodeUrl, invoke, isAgentCardResponse, isDataGetResponse, isDataListResponse, isListAgentsResponse, isListHumanMembersResponse, isMutationResponse, isObject, isOrgAdminStatus, isOrgContractGrants, isOrgPolicyMeta, isOrgPolicyView, isOrgUsageReport, isOrgWriters, isRoleAssignmentsResponse, isRoleGetResponse, isSecretListResponse, isSecretRecord, isSecretView, isUnsafeTrustServer, loadConfig, loadWasmComponent, manifestToTrustAnchor, maskKeyMaterial, mergeDiscoverDids, mergeGrants, metamask_get_address, metamask_sign, parseAgentAuthDoc, parseContractResponse, redactSecrets, redactSecretsFromJson, requestToWire, setEnvironment, setGlobalLogLevel, setNodeUrl, stringToBytes, tenantDidHex, toBaseUnits, validateConfig, validateTail, verifyDkgAttestation, verifyManifestSignature, verifyTdxQuote };
export type { ActivityCallerType, ActivityEntry, ActivityOutcome, ActivityReport, AdmitForOrgOptions, AgentAuthDoc, AgentCardRefInput, AgentCardResponse, AgentCardSetInput, AgentSummary, AssignRolesInput, AuthInput, BalanceRow, BoundGrant, ChargeReason, ClientAuth, ClientHandshake, ConfigValidationResult, ContractDescriptorFacets, ContractExecuteInput, ContractKind, ContractLifecycleStatus, ContractListDetailedOptions, ContractListDetailedPage, ContractPublishInput, ContractRegisterResult, ContractResponseSchema, ContractSetDescriptorInput, CreatePolicyInput, DataGetInput, DataGetResponse, DataListInput, DataListResponse, DelegationGrantKind, DelegationGrantRef, DeleteDataInput, DeleteGrantsInput, DeleteScopeInput, DetailedContract, Did, Direction, DkgAttestation, DkgVerifyResult, EmailOtpAuthInput, EmailOtpCredentials, Environment, EthAuthInput, ExecuteBusinessContractOptions, ExecuteOrgDataActionOptions, FetchTrustedManifestOptions, GetActivityLogOptions, GetUsageOptions, GrantCheckParams, GrantCheckResult, GranteeInput, GrantsGetInput, GrantsMergeResult, GuestToHostHandler, GuestToHostHandlers, HandshakeResult, HumanMember, InvokeOptions, InvokeRequest, JsonRpcRequest, JsonRpcResponse, KycPollCadence, KycPollOptions, KycStatus, KycStatusKind, ListAgentsInput, ListAgentsResponse, ListContractsParams, ListContractsResult, ListContractsScope, ListHumanMembersInput, ListHumanMembersResponse, ListedContract, Logger, MapCreateInput, MapLifecycleStatus, MapResponse, MapUpdateInput, MapVisibility, MemberRole, MutationResponse, OidcAuthInput, OidcCredentials, OrgAdminStatus, OrgContractGrants, OrgDataActionWire, OrgDataClientOptions, OrgPolicyMeta, OrgPolicyView, OrgUsageReport, OrgUsageReportInput, OrgWriters, OtpChannel, OtpMergeSuggestion, OtpRequestInput, OtpRequestResult, OtpVerifyInput, OtpVerifyResult, PayrollRunRequest, PeerQuoteResult, PolicyGetInput, PurgeOrgInput, QuoteVerifyResult, ReaderSet, RemoveMemberGrantsInput, RoleAssignments, RoleAssignmentsResponse, RoleDefinition, RoleGetResponse, RoleGrant, RoleIdInput, SdkConfig, SecretKind, SecretListInput, SecretListResponse, SecretNameInput, SecretRecord, SecretSetValueInput, SecretView, SecretWriteInput, SessionCrypto, SessionId, SetGrantsInput, SetRoleInput, SetWritersInput, SignedTrustManifest, StringEncoding, SubmitUserInputArgs, SubmitUserInputResult, T3nClientConfig, TenantAdmitProjection, TenantAdmitStatus, TenantBaseClient, TenantClientConfig, TenantContractExecuteInput, TenantContractPublishInput, TenantContractSetDescriptorInput, TenantExecutionSession, TenantMapCreateInput, TenantMapUpdatePatch, TenantMeResponse, TenantSdkEnvironment, TenantSelfAdmitResult, TenantStatus, TenantTargetOptions, TokenTxKind, Transport, TrustAnchor, TrustAnchorOrUnsafe, TrustManifestPayload, UnassignRolesInput, UnsafeTrustServer, UpdateMetaInput, UsageEntry, UsagePage, UsageReportSignature, UserInputProfile, UserUpsertErrorKind, Validity, WasmComponent, WasmNextResult, WireBlock, WorkloadAuthInput, WriteDataInput, WriterSet, WritersGetInput };
