...
import { serializeIpAddress, makeIpAddress, ipAddressConflict, isLoopbackIpAddress, } from "../sockets/address.js";
...
async function handleBind({ socketId, localAddress }) {
    ...
    const hasConflict = [...sockets].some(([id, { localAddress: boundAddress }]) => id !== socketId && boundAddress && ipAddressConflict(boundAddress, localAddress));
    if (hasConflict) {
...
async function handleConnect({ socketId, remoteAddress }) {
    const socket = sockets.get(socketId);
    const preserveLocalAddress = socket.connected &&
        socket.localAddress &&
