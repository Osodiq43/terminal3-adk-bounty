...
let WORKER = null;
function worker() {
    return (WORKER ??= new ResourceWorker(() => new Worker(new URL("../workers/filesystem-worker.bundle.js", import.meta.url))));
}
class Descriptor {
    ...
    static _create(handle, mode, fullPath, isDirectory = false) {
        const { read = false, write = false, fileIntegritySync = false, dataIntegritySync = false, requestedWriteSync = false, mutateDirectory = false, } = mode;
        const merged = {
            read,
            write,
            fileIntegritySync,
...
export function _setPreopens(entries) {
    ...
    Object.entries(entries).forEach(([virtualPath, hostPreopen]) => _addPreopen(virtualPath, hostPreopen));
}
/**
