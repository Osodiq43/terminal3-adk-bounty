...
import { _fieldsFromEntriesChecked, _readTrailersForTransport, _trailerResultFromEntries, } from "./fields.js";
let WORKER = null;
function worker() {
    return (WORKER ??= new ResourceWorker(() => new Worker(new URL("../workers/http-worker.bundle.js", import.meta.url))));
}
/**
...
export class HttpServer extends EventEmitter {
    ...
    async #serveLoop() {
        let next;
        const nextRequest = async () => await worker().run({
            op: "server-next",
            serverId: this.#serverId,
        });
