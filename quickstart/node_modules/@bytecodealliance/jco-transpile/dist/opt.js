...
export async function runOptimizeComponent(componentBytes, opts) {
    ...
    await Promise.all(coreModules.map(async (metadata) => {
        if (metadata.metaType.tag === 'module') {
            ...
            const optimized = await runWasmOptCLI(componentBytes.subarray(metadata.range[0], metadata.range[1]), args, opts);
            if (optimized === null) {
    ...
    const getChildren = (parentIndex) => {
        ...
        for (let i = 0; i < nodes.length; i++) {
            ...
            if (metadata.parentIndex === parentIndex) {
                ...
                metadata.children = getChildren(idx);
                metadata.sizeChange = (metadata.children ?? []).reduce((total, childMetadata) => {
    ...
    const sizeChange = componentTree.reduce((total, metadata) => {
        const { prevLEBLen, newLEBLen, sizeChange } = metadata;
        if (newLEBLen === undefined) {
            throw new Error('unexpectedly undefined new LEB len');
        }
        if (prevLEBLen === undefined) {
    ...
    const write = (metadata) => {
        const { prevLEBLen, range, optimized, children, sizeChange } = metadata;
        if (prevLEBLen === undefined) {
    ...
    componentTree.forEach((metadata) => write(metadata));
    // write remaining
 ...
 */
async function runWasmOptCLI(source, args, opts) {
