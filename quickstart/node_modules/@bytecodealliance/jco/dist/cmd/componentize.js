...
export async function componentize(jsSource, opts) {
    ...
    if (sourceType === "typescript-declaration") {
        throw new Error(`TypeScript declaration files cannot be componentized directly: ${jsSource}. Provide a .ts, .mts, .cts, or .tsx entry module instead.`);
    }
...
function normalizeBackend(backend = "starlingmonkey") {
    ...
    if (!normalized) {
        throw new Error(`Unknown componentization backend "${backend}". Expected one of: starlingmonkey, sm, quickjs, qjs.`);
    }
    return normalized;
}
function validateBackendOptions(backend, opts) {
    ...
    if (incompatible.length > 0) {
        throw new Error(`The ${incompatible.map((option) => `--${option.replace(/[A-Z]/g, (letter) => `-${letter.toLowerCase()}`)}`).join(", ")} option${incompatible.length === 1 ? " is" : "s are"} only supported by the starlingmonkey backend.`);
    }
}
/**
...
async function componentizeCJS(args) {
    ...
    if (useOldComponentizeJS) {
        ...
        // for fetch.
        console.error(`${styleText(["yellow", "bold"], "warning")} Falling back to componentize-js 0.19.3 because this component requests Preview 2 WASI packages older than 0.2.10. See https://bytecodealliance.github.io/jco/troubleshooting/common-issues.html#componentize-js-0193-fallback for details and upgrade steps.`);
