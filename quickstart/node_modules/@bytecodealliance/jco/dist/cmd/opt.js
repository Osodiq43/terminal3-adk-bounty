...
export async function optimizeComponent(componentBytes, opts) {
    ...
    try {
        ...
        let completed = 0;
        const spinnerText = () => `${styleText("cyan", `${completed} / ${coreModules.length}`)} Running Binaryen on WebAssembly Component Internal Core Modules \n`;
        if (showSpinner) {
            spinner = ora({
                color: "cyan",
                spinner: "bouncingBar",
        ...
        await Promise.all(coreModules.map(async (metadata) => {
            if (metadata.metaType.tag === "module") {
                ...
                metadata.optimized = await wasmOpt(componentBytes.subarray(metadata.range[0], metadata.range[1]), args, opts);
