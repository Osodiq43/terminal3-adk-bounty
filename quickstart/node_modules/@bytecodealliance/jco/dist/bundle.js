...
export async function bundleComponentSource(entryPath, options = {}) {
    ...
    try {
        ...
        if (assets.length > 0) {
            throw new Error(`Component bundling produced unsupported assets: ${assets.map((asset) => asset.fileName).join(", ")}`);
        }
        if (chunks.length !== 1 || !chunks[0].isEntry) {
            throw new Error(`Component bundling must produce exactly one entry chunk; received ${chunks.length} chunk(s). Dynamic or split chunks are not supported.`);
        }
...
function mergeExternal(configExternal, jcoExternal = []) {
    const external = [WASI_EXTERNAL, configExternal, ...jcoExternal].filter((option) => option !== undefined);
